...
class DDPMTrainer(BaseTrainer):
    def __init__(self, args):
        ...
        wandb.init(config = args , project='MOOD', name=args.model_name, group="DDP")
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            if self.debugging and step>10:
                break
            if self.project == 'mood':
                images = self.vqvae_model.encode_stage_2_inputs(batch["image"][tio.DATA].float().squeeze(dim=-1).to(self.device))
            else:
