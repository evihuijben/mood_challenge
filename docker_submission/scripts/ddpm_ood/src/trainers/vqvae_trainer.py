...
class VQVAETrainer:
    def __init__(self, args):
        ...
        for k, v in vars(args).items():
            print(f"  {k}: {v}")
        wandb.init(config = args , project='VQVAE', name=args.model_name, group="DDP")
        ...
        self.optimizer_d = torch.optim.Adam(params=self.discriminator.parameters(), lr=args.vqvae_learning_rate * 2)
        ...
        self.scheduler_g = torch.optim.lr_scheduler.LambdaLR(self.optimizer_g, lr_lambda=lambda1)
        self.scheduler_d = torch.optim.lr_scheduler.LambdaLR(self.optimizer_d, lr_lambda=lambda1)
    ...
    def save_checkpoint(self, path, epoch, save_message=None):
        ...
        if not self.ddp:
            ...
            print(save_message)
            torch.save(checkpoint, path)
    def check_nans(self, inputs):
        nan_mask = torch.isnan(inputs)
        nan_indices = torch.nonzero(nan_mask)
        if nan_indices.shape[0] > 0:
            raise ValueError("NaN values found in the following image:\n")
    def train(self, args):
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            if self.debugging and step>10:
                    break
            if self.project == 'mood':
                images = batch["image"][tio.DATA].float().squeeze(dim=-1).to(self.device)
            else:
                images = batch["image"].to(self.device)
            # check for nan in the input
            self.check_nans(images)
            ...
            wandb.log({
                ...
                "jukebox_loss": jukebox_loss.item(),
            ...
            if step== 0:
                ...
                for i in range(2):
                    ...
                    plt.imshow(images[sl].float().detach().cpu(), cmap="gray")
                    if i == 0:
                        plt.title("Image")
                    plt.subplot(2, 2, i * 2 + 2)
                    plt.imshow(reconstruction.float().detach()[sl].cpu(), cmap="gray")
                    if i == 0:
                        plt.title("Recon")
                plt.show()
                self.logger_val.add_figure(
                    tag="training_recons", figure=fig, global_step=self.global_step
                )
    ...
    def val_epoch(self, epoch):
        with torch.no_grad():
            ...
            for step, batch in progress_bar:
                ...
                # )
                wandb.log({
                "val_l1_loss": recons_loss.item(),	
                "val_generator_loss": total_generator_loss.item()})
                ...
                if step == 0:
                    ...
                    for i in range(2):
                        ...
                        if i == 0:
                            plt.title("Image")
                        plt.subplot(2, 2, i * 2 + 2)
                        plt.imshow(reconstruction.float()[sl].cpu(), cmap="gray")
                        if i == 0:
                            plt.title("Recon")
                    plt.show()
                    self.logger_val.add_figure(
                    ...
                    wandb.log({"val_recons": fig})




class VQVAEMoodTrainer:
    def __init__(self, args):
        # torch.autograd.set_detect_anomaly(True)
        ...
        # lambda1 = lambda epoch: 0.96 ** epoch
        # self.scheduler_g = torch.optim.lr_scheduler.LambdaLR(self.optimizer_g, lr_lambda=lambda1)
        self.scheduler_g = torch.optim.lr_scheduler.ExponentialLR(self.optimizer_g, gamma=0.9)
    ...
    def train_epoch(self, epoch):
        ...
        for step, batch in progress_bar:
            ...
            progress_bar.set_postfix(
                {
                    "l1_loss": self.L1_weight * l1_loss / (epoch_step),
                    "generator_loss": generator_epoch_loss / (epoch_step),
                    "discriminator_loss": adv_weight * discriminator_epoch_loss / (epoch_step),
                }
            )
        ...
        if  epoch >= 40:
            ...
            print('the new learning rate is {}'.format(round(self.scheduler_g.get_last_lr()[0], 9)))
            print('the new learning rate is {}'.format(round(self.scheduler_d.get_last_lr()[0], 9)))
        return epoch_loss
